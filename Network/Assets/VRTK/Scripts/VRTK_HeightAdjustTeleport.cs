...
namespace VRTK
{
    ...
    public class VRTK_HeightAdjustTeleport : VRTK_BasicTeleport
    {
        ...
        [Tooltip("Checks if the user steps off an object into a part of their play area that is not on the object then they are automatically " +
        	"teleported down to the nearest floor. The `Play Space Falling` option also works in the opposite way that if the user's headset is " +
        	"above an object then the user is teleported automatically on top of that object, which is useful for simulating climbing stairs without" +
        	" needing to use the pointer beam location. If this option is turned off then the user can hover in mid-air at the same y position of the " +
        	"object they are standing on.")]
        ...
        public float floorHeightTolerance = 0.0009f;
